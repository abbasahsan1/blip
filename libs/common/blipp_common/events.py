...
logger = logging.getLogger("blipp_common.events")


class EventBus:
    """
    Reusable NATS JetStream connection manager and typed publishing helpers
    for Blipp platform services and background workers.
    """
    ...
    async def connect(self, client_name: str = "blipp-service") -> bool:
        """
        ...
        logger.info(f"Connecting to NATS at {nats_url} (name={client_name})...")
        try:
    ...
    async def ensure_streams(self) -> None:
        ...
        - UPLOADS: subjects ['upload.>', 'transcode.>']
        ...
        stream_definitions: List[Dict[str, Any]] = [
            {
                ...
                "subjects": [self.settings.NATS_SUBJECT_UPLOADS, "transcode.>"],
            },
            {
